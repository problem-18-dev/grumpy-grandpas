...
func _get_configuration_warnings():
	...
	if children.size() == 0:
		return [
			"A DestructiblePolygon2D requires at least one Polygon2D child to define its initial polygon(s)."
		]
...
func update_bounds_and_area(polygon_2d: Polygon2D, polygon: PackedVector2Array) -> float:
	...
	polygon_2d.set_meta("bounds", Rect2(min_x, min_y, max_x - min_x, max_y - min_y))
			...
			if Geometry2D.is_polygon_clockwise(clipped_polygons[1]):
				...
				for i in boundary_size:
					...
					for j in hole_size:
						...
						if !Geometry2D \
								.clip_polyline_with_polygon(link1, clipped_polygons[0]) \
								.is_empty():
							continue

						if !Geometry2D \
								.intersect_polyline_with_polygon(link1, clipped_polygons[1]) \
								.is_empty():
							continue
						...
						for k in range(i + 1, boundary_size):
							...
							for l in hole_size:
								...
								if !Geometry2D \
										.clip_polyline_with_polygon(link2, clipped_polygons[0]) \
										.is_empty():
									continue

								if !Geometry2D \
										.intersect_polyline_with_polygon(link2, clipped_polygons[1]) \
										.is_empty():
									continue
